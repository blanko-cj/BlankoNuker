...
print(f'{colors["main"]}\n\n                
      ...
      r'                 | $$  \ $| $$/$$__  $| $$  | $| $$_  $$| $$  | $$' + "\n"
      r'                 | $$$$$$$| $|  $$$$$$| $$  | $| $$ \  $|  $$$$$$/' + "\n"
      r'                 |_______/|__/\_______|__/  |__|__/  \__/\______/ ' + "\n"
      '\n'
      r'                                                                  ' + "\n"
      r'                  /$$   /$$         /$$                           ' + "\n"
      r'                 | $$$ | $$        | $$                           ' + "\n"
      ...
      r'                 | $$\  $$| $$  | $| $$_  $$| $$_____| $$         ' + "\n"   
      r'                 | $$ \  $|  $$$$$$| $$ \  $|  $$$$$$| $$         ' + "\n"  
      r'                 |__/  \__/\______/|__/  \__/\_______|__/         ' + "\n"  
      "\n"
      "\n"
      "\n"
      f"{colors['white']}                           Author: {colors['main']}blanko.cj\n"
      f"{colors['white']}                           Version: {colors['main']}{version} {checkVersion()}\n"
      f"{colors['white']}                           GitHub: {colors['main']}https://github.com/blanko-cj/blankonuker\n\n{colors['white']}")
...
async def help(ctx):
    ...
    embed.set_author(name="Blanko Nuker",
                     url="https://github.com/blanko-cj/blankonuker")
    embed.add_field(
    ...
    embed.add_field(name="\u200b\nInfo",
                    value=f">>> **Blanko Nuker**\nMade by <@991772846957273139>\nVersion: {version} {checkVersion()}\nGitHub: https://github.com/blanko-cj/blankonuker\n", inline=False)
...
async def settings(ctx):
    ...
    embed.set_author(name="Blanko Nuker",
                     url="https://github.com/blanko-cj/blankonukerr")
    embed.add_field(
...
@commands.check(isOwner)
async def settingsActivity(ctx, action, activityType="playing", *, text=f"Blanko Nuker v{version}"):
    global config
    global activity
    if action == "set":
...
@commands.check(isWhitelisted)
async def nuke(ctx, ban: bool = True, text: str = "Blanko Nuker"):
    await msg_delete(ctx)
    ...
    try:
        ...
        embed.add_field(name="Nuked LOL",
                        value="By Blanko Nuker\nDownload: https://github.com/blanko-cj/blankonuker", inline=False)
        channel = await ctx.guild.create_text_channel(name="Blanko Nuker")
...
@commands.check(isWhitelisted)
async def renameEveryone(ctx, *, name="Blanko Nuker"):
    await msg_delete(ctx)
    for m in ctx.guild.members:
        if m.id not in owners:
            try:
...
@commands.check(isWhitelisted)
async def dmEveryone(ctx, *, msg="Blanko Nuker"):
    await msg_delete(ctx)
    for m in ctx.guild.members:
        if m.id not in owners:
            try:
...
@commands.check(isWhitelisted)
async def spamToAllChannels(ctx, amount: int = 50, *, text="@everyone Blanko Nuker"):
    await msg_delete(ctx)
...
@commands.check(isWhitelisted)
async def spamToCurrentChannel(ctx, amount: int = 50, *, text="@everyone Blanko Nuker"):
    await msg_delete(ctx)
    for i in range(amount):
        try:
...
@commands.check(isWhitelisted)
async def spamWithChannels(ctx, amount: int = 25, *, name="Blanko  Nuker"):
    await msg_delete(ctx)
    for i in range(amount):
        try:
...
@commands.check(isWhitelisted)
async def spamWithRoles(ctx, amount: int = 25, *, name="Blanko Nuker"):
    await msg_delete(ctx)
    for i in range(amount):
        try:
...
@commands.check(isWhitelisted)
async def editServerName(ctx, *, name="Blanko Nuker"):
    await msg_delete(ctx)
    try:
...
@commands.check(isWhitelisted)
async def getAdmin(ctx, *, rolename="Blanko Nuker"):
    await msg_delete(ctx)
    try:
...
async def reviveGuild(ctx, guildId: int = None):
    if guildId:
        ...
        try:
            await guild.create_text_channel(name="Blanko Nuker")
